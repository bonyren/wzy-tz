...
var EnterpriseModule = {
    ...
    convert:function(data){
        ...
        $.each(data.rows, function(i,v){
            var btns = [];
            btns.push('<a href="javascript:void(0);" class="btn size-MINI radius" onclick="QT.helper.view({url:\'<?=url('index/enterprises/view')?>?id='+v.id+'\',width:\'100%\',height:\'100%\',dialog:\'globel-dialog-div\'})" title="查看"><i class="fa fa-eye"></i>查看</a>');
            btns.push('<a href="javascript:void(0);" class="btn size-MINI radius" onclick="EnterpriseModule.progress(' + v.id + ')" title="进展"><i class="fa fa-flag">进展</i></a>');
            ...
            $(that.datagrid).datagrid('updateRow',{
                ...
                row: {
                    ...
                    founder: (v.founder in data.founders) ? '<a href="javascript:void(0)" onclick="QT.helper.view({url:\'<?=url('index/contacts/view')?>?id='+data.founders[v.founder].id+'\'})">'+data.founders[v.founder].name+'</a>' : '',
    ...
    progress:function(id){
        var href = '<?=url('index/ProgressLogs/light', ['readOnly'=>1,'category'=>\app\index\logic\ProgressLogs::INVESTED_PROGRESS_CATEGORY])?>';
