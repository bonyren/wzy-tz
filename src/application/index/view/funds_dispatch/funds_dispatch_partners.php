...
<script>
    $('#fundsDispatchPartnersAttachments').attaches({
        attachmentType:<?=\app\index\logic\Upload::ATTACH_FUND_DISPATCH_PARTNERS?>,
        externalId:<?=$ffiId?>,
        uiStyle:<?=\app\index\controller\Upload::ATTACHES_UI_DATAGRID_STYLE?>,
        readOnly:false
    });
    var fundsDispatchPartnersModule = {
        datagrid: '#fundsDispatchPartnersDatagrid',
        ...
        reload:function () {
            var that =this;
            var params = {};
            $(that.datagrid).datagrid('load', params);
        },
        editIndex:undefined,
        endEditing:function () {
            var that = fundsDispatchPartnersModule;
        ...
        onEndEdit:function (index, data, changes) {
            ...
            if(commonModule.IsEmptyObject(changes)){
                return false;
            }
            changes.p_id = data.p_id;
            $.post("<?=$urlHrefs['fundsDispatchPartnersSave']?>",
                changes,
                function(res){
                    if(!res.code){
                        ...
                        $(that.datagrid).datagrid('updateRow',{
                            index:index,
                            row:{
                                amount:oldrow[0].oldHtml,
                                fee:oldrow[1].oldHtml,
                                tax:oldrow[2].oldHtml
                            }
                        });
                    }else{
                        that.refresh();
        ...
        auto:function(){
            var that = this;
            $.messager.confirm('提示', '确定执行默认分配吗?<br>（分配总额 * 基金份额占比）', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
                $.post('<?=url('index/fundsDispatch/fundsDispatchPartnersDefault')?>', {ffiId:'<?=$ffiId?>'}, function(res){
