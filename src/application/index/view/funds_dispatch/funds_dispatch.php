...
<script>
var fundsDispatchModule = {
    dialog:'#globel-dialog2-div',
    datagrid:'#fundsDispatchDatagrid',
    exitTypes:<?=json_encode(\app\index\logic\Defs::ENTERPRISE_EXIT_TYPES, JSON_UNESCAPED_UNICODE)?>,
    operate:function(val, row){
        var btns = [];
        btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsDispatchModule.dispatch(' + row.ffi_id + ','+ row.enterprise_id + ')" title="合伙人派发"><i class="fa fa-share-alt fa-lg"></i></a>');
        return btns.join(' ');
    },
    transExitType:function(v){
        return fundsDispatchModule.exitTypes[v];
    },
    ...
    dispatch:function(ffiId, enterpriseId){
        var that = this;
        var href = '<?=$urlHrefs['fundsDispatchPartners']?>';
        href += href.indexOf('?') != -1 ? '&ffiId=' + ffiId : '?ffiId='+ffiId;
        href += href.indexOf('?') != -1 ? '&enterpriseId=' + enterpriseId : '?enterpriseId='+enterpriseId;
        $(that.dialog).dialog({
            title: '合伙人分配方案',
