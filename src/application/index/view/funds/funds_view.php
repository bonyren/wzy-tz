<div class="easyui-tabs" data-options="fit:true,tabPosition:'bottom',justified:false,border:false,tabWidth:100,headerWidth:100">
    ...
    <div title="基本信息" data-options="cache:false,href:'<?=$urlHrefs['fundsBasic']?>',iconCls:'fa fa-info-circle',border:false"></div>
    ...
    <div title="基金募集" data-options="iconCls:'fa fa-magnet',href:'<?=$urlHrefs['fundsCollect']?>',border:false,selected:<?=$bindValues['statusFilter'] == \app\index\logic\Funds::FUND_COLLECT_STATUS?'true':'false'?>">
    </div>
    <?php } ?>
    ...
    <div title="基金投资" data-options="iconCls:'fa fa-share-alt',border:false,selected:<?=$bindValues['statusFilter'] == \app\index\logic\Funds::FUND_INVEST_STATUS?'true':'false'?>">
    ...
    <div title="基金管理" data-options="iconCls:'fa fa-wrench',border:false,selected:<?=$bindValues['statusFilter'] == \app\index\logic\Funds::FUND_MANAGE_STATUS?'true':'false'?>">
    ...
    <?php } ?>
    <?php if($bindValues['statusFilter'] == \app\index\logic\Funds::FUND_ALL_STATUS || $bindValues['statusFilter'] >= \app\index\logic\Funds::FUND_MANAGE_STATUS){ ?>
    <div title="基金退出" data-options="iconCls:'fa fa-sign-out',border:false,selected:<?=$bindValues['statusFilter'] == \app\index\logic\Funds::FUND_EXIT_STATUS?'true':'false'?>">
        <div class="easyui-tabs" data-options="fit:true,justified:false">
            <div title="收益分配" data-options="cache:false,href:'<?=$urlHrefs['fundsDispatch']?>',iconCls:'fa fa-users',border:false"></div>
        </div>
    </div>
    <?php } ?>
    <div title="操作日志" data-options="iconCls:'fa fa-align-justify',cache:false,href:'<?=$urlHrefs['fundsLog']?>',border:false"></div>
</div>
