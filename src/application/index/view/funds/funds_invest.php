...
<script>
    var fundsInvestModule = {
        dialog:'#globel-dialog-div',
        datagrid:'#fundsInvestDatagrid',
        operate:function(val, row){
            var btns = [];
            <?php if($loginUserMenuPriv == CommonDefs::AUTHORIZE_READ_WRITE_TYPE){ ?>
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsInvestModule.view(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="查看"><i class="fa fa-eye">查看</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsInvestModule.progress(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="进展"><i class="fa fa-flag">进展</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsInvestModule.goManage(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="进入管理"><i class="fa fa-arrow-down">进入管理</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsInvestModule.backCollect(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="返回募集"><i class="fa fa-arrow-up">返回募集</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsInvestModule.manageInvest(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="投资管理"><i class="fa fa-share-alt">投资管理</i></a>');
        ...
        search:function(){
            ...
            $.each($("#fundsInvestToolbarForm").serializeArray(), function() {
        ...
        manageInvest:function(fundId, name){
            var that = this;
        ...
        goManage:function(fundId, name){
            var that = this;
            $.messager.confirm('提示', '确认[' + name + ']投资完成，进如基金管理阶段吗?', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
                $.post('<?=url('index/Funds/changeStatus')?>', {
                        fundId:fundId,
                        fromStatus:<?=\app\index\logic\Funds::FUND_INVEST_STATUS?>,
        ...
        backCollect:function(fundId, name){
            var that = this;
            $.messager.confirm('提示', '确认[' + name + ']返回募集阶段吗?', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
                $.post('<?=url('index/Funds/changeStatus')?>', {
                        fundId:fundId,
                        fromStatus:<?=\app\index\logic\Funds::FUND_INVEST_STATUS?>,
                        toStatus:<?=\app\index\logic\Funds::FUND_COLLECT_STATUS?>
                    },
                    function(res){
                        $.messager.progress('close');
                        if(!res.code){
