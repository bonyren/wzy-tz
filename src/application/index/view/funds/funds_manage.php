...
<script>
    var fundsManageModule = {
        dialog:'#globel-dialog-div',
        datagrid:'#fundsManageDatagrid',
        operate:function(val, row){
            var btns = [];
            <?php if($loginUserMenuPriv == CommonDefs::AUTHORIZE_READ_WRITE_TYPE){ ?>
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.view(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="查看"><i class="fa fa-eye">查看</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.progress(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="进展"><i class="fa fa-flag">进展</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.edit(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="编辑"><i class="fa fa-pencil-square-o">编辑</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.goExit(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="注销"><i class="fa fa-window-close-o">注销</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.backInvest(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="返回投资"><i class="fa fa-arrow-up">返回投资</i></a>');
            <?php }else if($loginUserMenuPriv == CommonDefs::AUTHORIZE_READ_ONLY_TYPE){ ?>
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.view(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="查看"><i class="fa fa-eye">查看</i></a>');
            <?php } ?>
            return btns.join(' ');
        },
        operateFinance:function(val, row, index){
            var btns = [];
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.finance(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="财务管理"><span class="fa fa-money">管理</span></a>');
            return btns.join(' ');
        },
        operateDispatch:function(val, row, index){
            var btns = [];
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.dispatch(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="收益分配"><span class="fa fa-share-alt">管理</span></a>');
            return btns.join(' ');
        },
        operateArchive:function(val, row, index){
            var btns = [];
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.archive(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="文件管理"><span class="fa fa-file">管理</span></a>');
            return btns.join(' ');
        },
        /*
        operateEvent:function(val, row, index){
            var btns = [];
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="fundsManageModule.event(' + row.fund_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="税务">管理</a>');
            return btns.join(' ');
        },*/
        ...
        search:function(){
            ...
            $.each($("#fundsManageToolbarForm").serializeArray(), function() {
        ...
        edit:function(fundId, name){
            ...
            fundsCommonModule.edit(href, name + ' - 基金管理', function(){
                that.reload();
            });
        },
        finance:function(fundId, name){
            var that = this;
            var href = '<?=$urlHrefs['fundsFinance']?>';
            href += href.indexOf('?') != -1 ? '&fundId=' + fundId : '?fundId='+fundId;
            $(that.dialog).dialog({
                title: name + ' - 财务',
        ...
        dispatch:function(fundId, name){
            var that = this;
            var href = '<?=$urlHrefs['fundsDispatch']?>';
            href += href.indexOf('?') != -1 ? '&fundId=' + fundId : '?fundId='+fundId;
            $(that.dialog).dialog({
                title: name + ' - 收益分配',
                iconCls: iconClsDefs.edit,
        ...
        archive:function(fundId, name){
            var that = this;
            var href = '<?=$urlHrefs['fundsManageArchives']?>';
            href += href.indexOf('?') != -1 ? '&fundId=' + fundId : '?fundId='+fundId;
            $(that.dialog).dialog({
                title: name + ' - 文件',
                iconCls: iconClsDefs.edit,
        ...
        event:function(fundId, name){
            var that = this;
            var href = '<?=$urlHrefs['fundsManageEvent']?>';
            href += href.indexOf('?') != -1 ? '&fundId=' + fundId : '?fundId='+fundId;
            $(that.dialog).dialog({
                title: name + ' - 事件',
                iconCls: iconClsDefs.edit,
        ...
        goExit:function(fundId, name){
            var that = this;
            $.messager.confirm('提示', '确认[' + name + ']进如基金退出阶段吗?', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
                $.post('<?=url('index/Funds/changeStatus')?>', {
                        fundId:fundId,
                        fromStatus:<?=\app\index\logic\Funds::FUND_MANAGE_STATUS?>,
                        toStatus:<?=\app\index\logic\Funds::FUND_EXIT_STATUS?>
                    },
                    function(res){
        ...
        backInvest:function(fundId, name){
            var that = this;
            $.messager.confirm('提示', '确认[' + name + ']返回投资阶段吗?', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
                $.post('<?=url('index/Funds/changeStatus')?>', {
                        fundId:fundId,
