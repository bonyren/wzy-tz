        ...
        <form id="progressLogsAddForm_<?=$uniqid?>" method="post">
            <table class="table-form">
                ...
                <tr>
                    ...
                    <td class="field-input">
                        <input id="entry_<?=$uniqid?>" class="easyui-textbox" name="infos[entry]" data-options="label:'',
                            width:'100%',
                            multiline:true,
                            disabled:false,
                ...
                <tr class="form-tools">
                    <td colspan="2" align="center">
                        <a href="#" class="easyui-linkbutton" data-options="onClick:function(){ progressLogsModule_<?=$uniqid?>.add(); },iconCls:'fa fa-save'">保存</a>
                    </td>
                </tr>
            </table>
        </form>
...
var progressLogsModule_<?=$uniqid?> = {
        ...
        data.rows.forEach(function(v,i){
            $.get('<?=url('index/Upload/viewAttaches')?>',{
                ...
                $(that.datagrid).datagrid('updateRow',{
                    index: i,
                    row: {
                        title:'<button type="button" class="btn btn-link" onclick="progressLogsModule_<?=$uniqid?>.view('+v.progress_log_id+')">'+v.title+'</button>',
                        files:res
                    }
                });
            });
    ...
    operate:function(val, row){
        var btns = [];
        btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="progressLogsModule_<?=$uniqid?>.edit(' + row.progress_log_id + ')" title="编辑"><i class="fa fa-pencil-square-o fa-lg"></i></a>');
        btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="progressLogsModule_<?=$uniqid?>.del(' + row.progress_log_id + ')" title="删除"><i class="fa fa-trash-o fa-lg"></i></a>');
        return btns.join(' ');
    },
    formatShowTimeline:function(val, row){
        if(val == 1){
    ...
    edit:function(id){
        ...
        },<?=$loginMobile?"'90%'":800?>,"95%",'progress_log_edit_dialog');
    },
