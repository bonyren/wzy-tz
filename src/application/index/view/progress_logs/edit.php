...
var <?=JVAR?> = {
    ...
    onSubtypeSelect:function(row){
        var that = <?=JVAR?>;
        $('#F<?=UNIQID?>').find('.progress_logs_extra_inputs').each(function(){
            if (row.value != '' && that.subtypes[row.value].flag == $(this).attr('flag')) {
                $(this).removeClass('hidden');
            } else {
                $(this).addClass('hidden');
            }
        });
        if (row.value == '' || that.subtypes[row.value].flag != 'meeting_res') {
            if (that.init) {
                $('#MR<?=JVAR?>').combobox('clear');
            }
        }
        that.init = 1;
    }
