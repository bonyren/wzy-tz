<form method="post" style="width: 100%;height: 100%;">
    <table class="table-form">
        ...
        <tr>
            ...
            <td class="field-input">
                <input id="change-logs-desc-textbox-<?=$uniqid?>" class="easyui-textbox" name="infos[desc]" data-options="label:'',
                    required:true,
                    width:'100%',
                    height:'auto',
                    multiline:true,
                    validType:['length[1,255]']" prompt="请输入描述" />
            </td>
        </tr>
        <tr>
...
<script type="text/javascript">
    var changeLogsAttacheIds_<?=$uniqid?> = [];
    var changeLogsAddModule_<?=$uniqid?> = {
        onAttachmentsUploaded:function(files){
            $.each(files, function(i,v){
                changeLogsAttacheIds_<?=$uniqid?>.push(v.attachment_id);
            });
            $('#changeLogsAttacheIds-<?=$uniqid?>').val(changeLogsAttacheIds_<?=$uniqid?>.join(','));
        }
    };
    $(function(){
        $('#change-logs-attaches-<?=$uniqid?>').attaches({
            attachmentType:<?=\app\index\logic\Upload::ATTACH_FUND_CHANGE_LOGS?>,
            externalId:0,
            uiStyle:<?=\app\index\controller\Upload::ATTACHES_UI_DATAGRID_STYLE?>,
            readOnly:false,
            callback:'changeLogsAddModule_<?=$uniqid?>.onAttachmentsUploaded'
        });
        $.parser.onComplete = function(context){
            $("#change-logs-desc-textbox-<?=$uniqid?>").textbox('autoHeight');
        }
    });
</script>