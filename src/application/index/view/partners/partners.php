...
<div id="partnersToolbar" class="p-1">
    <div>
        <a href="#" class="easyui-linkbutton" data-options="onClick:function(){ partnersModule.add(); },iconCls:iconClsDefs.add">添加</a>
    </div>
...
<script>
    var partnersModule = {
        ...
        datagrid:'#partnersDatagrid',
        operate:function(val, row){
            var btns = [];
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="partnersModule.edit(' + row.p_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="编辑"><i class="fa fa-pencil-square-o">编辑</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="partnersModule.delete(' + row.p_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="删除"><i class="fa fa-trash-o">删除</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="partnersModule.changePwd(' + row.p_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="重置登录帐号"><i class="fa fa-key">登录帐号</i></a>');
            btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="partnersModule.progress(' + row.p_id + ',\'' + GLOBAL.func.escapeChar(row.name) + '\')" title="进展"><i class="fa fa-flag">进展</i></a>');
            return btns.join(' ');
        },
        formatStatus:function(val, row){
            return <?=json_encode(\app\index\logic\Defs::$partnerStatusHtmlDefs, JSON_UNESCAPED_UNICODE)?>[val];
        },
        reload:function(){
        ...
        search:function(){
            ...
            $.each($("#partnersToolbarForm").serializeArray(), function() {
        ...
        reset:function(){
            ...
            $("#partnersToolbarForm").form('reset');
            that.load();
        },
        add:function(){
            var that = this;
        ...
        edit:function(pId, name){
            var that = this;
            var href = '<?=$urlHrefs['edit']?>';
            href += href.indexOf('?') != -1 ? '&pId=' + pId : '?pId='+pId;
            $(that.dialog).dialog({
                title: name + ' - 修改合伙人',
        ...
        delete:function(pId, name){
            var that = this;
            var href = '<?=$urlHrefs['delete']?>';
            href += href.indexOf('?') != -1 ? '&pId=' + pId : '?pId='+pId;
            $.messager.confirm('提示', name + ' - 确认删除吗?', function(result){
        ...
        view:function(pId, name){
            var that = this;
            var href = '<?=$urlHrefs['view']?>';
            href += href.indexOf('?') != -1 ? '&pId=' + pId : '?pId='+pId;
            $(that.dialog).dialog({
                title: name + ' - 查看合伙人',
                iconCls: 'fa fa-eye',
        ...
        changePwd:function(pId, name){
            var that = this;
            var href = '<?=$urlHrefs['password']?>';
            href += href.indexOf('?') != -1 ? '&pId=' + pId : '?pId='+pId;
            $(that.dialog).dialog({
                title: '修改合伙人登录帐号 - ' + name,
                iconCls: iconClsDefs.edit,
        ...
        progress:function(pId, name){
            var that = this;
            var href = '<?=$urlHrefs['partnersProgress']?>';
            href += href.indexOf('?') != -1 ? '&externalId=' + pId : '?externalId='+pId;
            $(that.dialog).dialog({
                title: name + ' - 合伙人进展',
