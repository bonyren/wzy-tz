...
var <?=JVAR?> = {
    ...
    convert:function(data){
        ...
        var that = <?=JVAR?>, records = [];
        $.each(data.rows, function(i,v){
            records.push(v.id);
        });
        $.post('<?=$urls['show_tags']?>',{record_id:records},function(tags){
            $.each(data.rows, function(i,v){
                var btns = [];
                <?php if($loginUserMenuPriv == CommonDefs::AUTHORIZE_READ_WRITE_TYPE){ ?>
                btns.push('<a href="javascript:void(0);" class="btn btn-outline-info size-MINI radius my-1" onclick="QT.helper.view({url:\'<?=url('contacts/view')?>?id='+v.id+'\'})" title="查看"><i class="fa fa-eye"></i>查看</a>');
                btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="<?=JVAR?>.edit(' + v.id + ')" title="编辑"><i class="fa fa-pencil-square-o">编辑</i></a>');
                ...
                btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="<?=JVAR?>.password(' + v.id + ',\'' + GLOBAL.func.escapeChar(v.name) + '\')" title="帐号密码"><i class="fa fa-key">帐号密码</i></a>');
                ...
                $(that.datagrid).datagrid('updateRow',{
                    ...
                    row: {
                        ...
                        enterprise: (v.enterprise_id in data.enterprise) ? data.enterprise[v.enterprise_id] : '',
                        description: v.description ? '<div class="desc-limiter">'+v.description+'</div>' : '',
                        tags:typeof tags[v.id] == "undefined" ? '' : tags[v.id],
                        btns:btns.join(' ')
                    }
                });
            });
    ...
    edit:function(id){
        var that = this,
            href = '<?=$urls['edit']?>?id='+(id?id:'');
        QT.helper.dialog(id?'编辑':'添加',href,true,function(){
            that.reload();
        },
        <?=$loginMobile?"'90%'":900?>);
    },
    ...
    password:function(id, name){
        var that = this;
        var href = '<?=url('contacts/password')?>?id='+id;
        QT.helper.dialog('设置登录帐号（' + name + '）',href,true,function($dialog){
            var $form = $('#contact-password-form');
