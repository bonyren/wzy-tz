<form method="post" id="F<?=UNIQID?>">
    <table class="table-form" cellpadding="5">
        ...
        <tr>
            ...
            <td>
                <?php echo \app\index\service\View::tagger(4,$row['id'],'',[
                    'name'=>'tags[4]',
                    'title' => '个人标签',
                ]); ?>
            </td>
        ...
        <tr>
            ...
            <td>
                <input name="description"
                       ...
                       class="easyui-textbox auto-height" multiline="true" style="width:100%;">
            </td>
        </tr>
        <tr>
            ...
            <td>
                <div class="easyui-panel" data-options="
                    href:'<?=url('index/upload/attaches',['attachmentType'=>8,'externalId'=>intval($row['id']),'uiStyle'=>\app\index\controller\Upload::ATTACHES_UI_LIGHT_STYLE]).($row['id'] ? '' : '&callback=GLOBAL.HelperDialog.uploaded')?>',
                    border:false,
                    minimizable:false,
                    maximizable:false">
                </div>
...
GLOBAL.HelperDialog = {
    ...
    uploaded:function(files){
        var that = GLOBAL.HelperDialog;
        $.each(files, function(i,v){
            that.pendingFiles.push(v.attachment_id);
        });
        $('#contacts_pending_files').val(that.pendingFiles.join(','));
    },
