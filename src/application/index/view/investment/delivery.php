...
var deliveryView = {
    investmentId: <?=$row['id']?>,
    save:function(id){
        var $form = $('#investment-base-info-'+id);
        if (!$form.form('validate')) {
            return false;
        }
        $.messager.progress({text:'处理中，请稍候...'});
        $.post('<?=url('investment/add')?>?id='+id,$form.serialize(),function(res){
    ...
    remove:function(id){
        $.messager.confirm('提示','确定删除该轮投资吗？',function (y) {
            if(!y){
                return false;
            }
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('investment/remove')?>',{id:id},function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
                    $.app.method.tip('提示', res.msg);
                    $('#enterprise_edit_view').tabs('getSelected').panel('refresh');
                }
            },'json');
        })
    },
    reload:function(){
        $('#enterprise-invested-tabs').tabs('getSelected').panel('refresh');
    }
}
</script>