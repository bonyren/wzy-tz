...
<script type="text/javascript">
    var businessRegProxyModule = {
        dialog:   '#globel-dialog-div',
        datagrid: '#businessRegProxyDatagrid',
        //工具栏
        toolbar: [
            { text: '添加', iconCls: iconClsDefs.add, handler: function(){businessRegProxyModule.add();} },
            { text: '刷新', iconCls: 'fa fa-refresh', handler: function(){businessRegProxyModule.refresh();} }
        ],
        operate: function(val, row ,index){
            var btn = [];
            btn.push('<a href="javascript:;" onclick="businessRegProxyModule.edit('+row.id+')"><i class="fa fa-pencil-square-o fa-lg"></i></a>');
            btn.push('<a href="javascript:;" onclick="businessRegProxyModule.delete('+row.id+')"><i class="fa fa-trash-o fa-lg"></i></a>');
            return btn.join(' | ');
        },
        refresh: function(){
    ...
    $.map(options, function(value, key){
        if(key=='toolbar'){
            console.log('toolbar');
            return businessRegProxyModule.toolbar;
        }
        return value;
    });*/
    for(var key in options){
        if(key=='toolbar'){
            options[key] = businessRegProxyModule.toolbar;
        }
    }
    $('#businessRegProxyDatagrid').datagrid(options);
</script>