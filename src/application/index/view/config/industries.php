...
var <?=JVAR?> = {
    ...
    del:function(){
        ...
        if (!node) {
            $.app.method.tip('提示', '请先选中目标数据', 'error');
            return;
        }
        $.messager.confirm('提示','确定要删除该数据吗？',function(y){
            if(!y){
                return false;
            }
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('config/delIndustry')?>', {id:node.id}, function(res){
    ...
    save:function(editId){
        ...
        var $dialog = QT.helper.genDialogId();
        var href = '<?=url('config/editIndustry')?>?id='+id+'&pid='+default_pid;
        $dialog.dialog({
            title: id ? '编辑节点' : '添加节点',
            width: <?=$loginMobile?"'90%'":300?>,
            height: '50%',
            cache: false,
            href: href,
            modal: true,
