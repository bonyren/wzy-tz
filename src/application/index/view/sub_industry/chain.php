<div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'west',minWidth:200" style="width:20%;" class="hide-tree-icon">
        <div style="margin-top:2px;margin-left:2px;height:30px;">
            <a href="#" class="easyui-linkbutton" iconCls="fa fa-plus-square-o" data-options="plain:true,
                onClick:function(){subIndustryChainModule.add();}">添加
            </a>
            <a href="#" class="easyui-linkbutton" iconCls="fa fa-pencil-square-o" data-options="plain:true,
                onClick:function(){subIndustryChainModule.edit();}">修改
            </a>
            <a href="#" class="easyui-linkbutton" iconCls="fa fa-trash-o" data-options="plain:true,
...
<script>
    var subIndustryChainModule = {
        tree:'#subIndustryChainTree',
        currentSubIndustryChainId: 0,
        graph:function(){
            QT.helper.view({
                title:'行业链图谱',
                dialog:'industries_graph',
                width:'100%',
                height:'100%',
                url:'<?=url('index/SubIndustry/chainGraph')?>?industry_id=<?=$subIndustryId?>',
            });
        },
        formatText:function(node){
            //console.log(node);
            if(node.enterprise_count == 0){
                return node.text;
            }else{
                return node.text + ' [<span class="text-success"><strong>' + node.enterprise_count + '</strong></span>]';
            }
        },
        init:function(){
            var that = subIndustryChainModule;
            if(that.currentSubIndustryChainId == 0) {
                //default
                ...
                while(node = cloneNodes.shift()){
                    if(node.id == that.currentSubIndustryChainId){
        ...
        add:function(){
            ...
            if(!parentNode){
                $.app.method.tip('提示信息', "请选择上层节点", 'error');
                return;
            }
            var parentId = parentNode.id;
            var href = '<?=$urlHrefs['subIndustryChainAdd']?>';
        ...
        edit:function(){
            ...
            if(!selectedNode){
                $.app.method.tip('提示信息', "请选择要修改的节点", 'error');
                return;
            }
            var subIndustryChainId = selectedNode.id;
            if(subIndustryChainId == 0){
                return;
            }
            var href = '<?=$urlHrefs['subIndustryChainUpdate']?>';
            href += href.indexOf('?') != -1 ? '&subIndustryChainId=' + subIndustryChainId : '?subIndustryChainId='+subIndustryChainId;
            var prompt = $.messager.prompt({
                title: '提示',
                msg: '请输入产业链节点名称:',
                fn: function(name){
        ...
        delete:function(){
            ...
            var selectedNode = $(that.tree).tree('getSelected');
            if(!selectedNode){
                $.app.method.tip('提示信息', "请选择要删除的节点", 'error');
                return;
            }
            var subIndustryChainId = selectedNode.id;
            if(0 == subIndustryChainId){
                $.app.method.tip('提示信息', "不允许删除根节点", 'error');
                return;
            }
            var childNodes = $(that.tree).tree('getChildren', selectedNode.target);
            if(childNodes.length != 0){
                $.app.method.tip('提示信息', "请先删除该节点下的子节点", 'error');
                return;
            }
            var href = '<?=$urlHrefs['subIndustryChainDelete']?>';
            href += href.indexOf('?') != -1 ? '&subIndustryChainId=' + subIndustryChainId : '?subIndustryChainId='+subIndustryChainId;
        ...
        onSelected:function(node){
            var that = subIndustryChainModule;
            subIndustryChainModule.currentSubIndustryChainId = node.id;
            subIndustryEnterprisesModule.load(node.id);
        }
    };
    /******************************************************************************************************************/
    var subIndustryEnterprisesModule = {
        dialog:'#globel-dialog-div',
        datagrid:'#subIndustryEnterprisesDatagrid',
        operate:function(val, row){
            var btns = [];
            btns.push('<a href="javascript:;" onclick="subIndustryEnterprisesModule.delete(' + row.id + ')" title="删除">删除</a>');
            btns.push('<a href="javascript:;" onclick="subIndustryEnterprisesModule.editEnterprise(' + row.eid + ')" title="编辑企业">编辑企业</a>');
            return btns.join(' | ');
        },
        reload:function(){
            $(this.datagrid).datagrid('reload');
        },
        load:function(subIndustryChainId){
            $(this.datagrid).datagrid('load', {subIndustryChainId:subIndustryChainId});
        },
        add:function(eids){
            var that = subIndustryEnterprisesModule;
            if($.trim(eids) == ''){
                return;
            }
            var currentSubIndustryChainId = subIndustryChainModule.currentSubIndustryChainId;
            if(currentSubIndustryChainId == 0){
                $.app.method.alertWarning(null, '请选择产业链上的节点');
                return;
            }
            var href = '<?=$urlHrefs['subIndustryChainEnterpriseAdd']?>';
            href += href.indexOf('?') != -1 ? '&subIndustryChainId=' + currentSubIndustryChainId : '?subIndustryChainId='+currentSubIndustryChainId;
            $.messager.progress({text:'处理中，请稍候...'});
            $.post(href, {eids:eids}, function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
                    $.app.method.tip('提示', res.msg, 'info');
                    subIndustryChainModule.reload();
                }
            }, 'json');
        },
        delete:function(id){
            var that = subIndustryEnterprisesModule;
            var href = '<?=$urlHrefs['subIndustryChainEnterpriseDelete']?>';
            $.messager.confirm('提示信息', '确定要删除吗？', function(result){
                if(!result) return false;
                $.messager.progress({text:'处理中，请稍候...'});
        ...
        editEnterprise:function(eid){
            var href = '<?=url('index/Enterprises/edit')?>?id=' + (eid?eid:'');
            QT.helper.view({
                url:href,
                title:'编辑项目',
                width:'100%',
