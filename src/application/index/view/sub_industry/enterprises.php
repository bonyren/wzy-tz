...
var <?=JVAR?> = {
    ...
    toolbar:'#<?=TOOLBAR_ID?>',
    mapPositions:<?=json_encode(\app\index\logic\Defs::INDUSTRY_POSITIONS,JSON_UNESCAPED_UNICODE)?>,
    ...
    hywz:function(val,row,idx){
        var that = <?=JVAR?>;
        if (val in that.mapPositions) {
            return that.mapPositions[val];
        } else {
            return '';
        }
    },
    ...
    addProject:function(eids){
        $.messager.progress({text:'处理中，请稍候...'});
        $.post('<?=url('index/SubIndustry/addEnterprises')?>', {iid:'<?=$iid?>',eids:eids}, function(res){
    ...
    edit:function(id){
        var that = this;
        var href = '<?=url('index/SubIndustry/setEnterprise')?>?id='+id;
        var $dialog = QT.helper.genDialogId('set-industry-enterprise');
        $dialog.dialog({
            title: '编辑',
    ...
    remove:function(id){
        ...
        $.messager.confirm('提示', '确认删除吗?', function(result){
            ...
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('index/SubIndustry/delEnterprises')?>', {id:id}, function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
