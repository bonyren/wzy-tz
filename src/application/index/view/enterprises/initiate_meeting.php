...
var <?=JVAR?> = {
    ...
    uploaded:function(files){
        ...
        $('#meeting_pending_files').val(that.pendingFiles.join(','));
    },
    submit:function(){
        var $form = $('#F<?=UNIQID?>');
        ...
        var url = '<?=$_request_url?>';
        <?php if(!empty($bind['meeting'])){ ?>
            url = GLOBAL.func.addUrlParam(url, 'meeting_id', '<?=$bind['meeting']['id']?>');
        <?php } ?>
        $.post(url, $form.serialize(), function(res){
                ...
                if(!res.code){
                    ...
                    try {
                        var callback = eval('<?=$_GET['callback']?>');
