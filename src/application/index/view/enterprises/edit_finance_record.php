...
var <?=JVAR?> = {
    ...
    detail:<?=json_encode($info['detail'],JSON_UNESCAPED_SLASHES)?>,
    pendingFiles:[],
    upload1:function(files){
        <?php if (!$info['id']): ?>
        var that = <?=JVAR?>;
        $.each(files, function(i,v){
            that.pendingFiles.push(v.attachment_id);
        });
        $('#fid1<?=UNIQID?>').val(that.pendingFiles.join(','));
        <?php endif; ?>
    },
    upload2:function(files){
        <?php if (!$info['esid']): ?>
        var that = <?=JVAR?>;
        $.each(files, function(i,v){
            $('#fid2<?=UNIQID?>').val(v.attachment_id);
        });
        <?php endif; ?>
    },
    del:function(el){
    ...
    add1:function(row){
        if ($('#T1<?=UNIQID?> tr').length > 2) {
            if (!$('#F<?=UNIQID?>').form('validate')) {
                return false;
            }
            var del = '<a href="javascript:void(0)" onclick="<?=JVAR?>.del(this)">删除</a></td>';
        } else {
            var del = '';
        }
        var id = QT.util.uuid();
        if (!row || $.isEmptyObject(row)) {
            row = {who:'',amount:'',stock_total:'',stock_ratio:''};
        }
        var tr = '<tr id="'+id+'">\
    ...
    add2:function(row){
        ...
        var id = QT.util.uuid();
        if (!row || $.isEmptyObject(row)) {
            row = {from:'',to:'',amount:'',stock_total:'',stock_ratio:''};
        }
        var tr = '<tr id="'+id+'">\
    ...
    viewes:function(){
        var url = '<?=url('enterprises/shareholdersEdit',['id'=>$info['esid'],'readonly'=>1])?>';
        QT.helper.view( { url:url,width:800,height:'80%',dialog:'view-shareholders' } );
    },
    ctype:function(v,o,isInit){
        var that = <?=JVAR?>;
        that.type = v;
        $('#BTN<?=UNIQID?>').removeClass('hidden');
        if (v == '1') {
            $('#T1<?=UNIQID?>').removeClass('hidden');
            $('#T2<?=UNIQID?>').addClass('hidden');
            $('#T1<?=UNIQID?>').find('.easyui-textbox').textbox('enableValidation');
            $('#T1<?=UNIQID?>').find('.easyui-numberbox').numberbox('enableValidation');
            $('#T2<?=UNIQID?>').find('.easyui-textbox').textbox('disableValidation');
            $('#T2<?=UNIQID?>').find('.easyui-numberbox').numberbox('disableValidation');
        } else {
            $('#T1<?=UNIQID?>').addClass('hidden');
            $('#T2<?=UNIQID?>').removeClass('hidden');
            $('#T2<?=UNIQID?>').find('.easyui-textbox').textbox('enableValidation');
            $('#T2<?=UNIQID?>').find('.easyui-numberbox').numberbox('enableValidation');
            $('#T1<?=UNIQID?>').find('.easyui-textbox').textbox('disableValidation');
            $('#T1<?=UNIQID?>').find('.easyui-numberbox').numberbox('disableValidation');
        }
        if (that.isAdd || (!isInit && $('#T'+v+'<?=UNIQID?> tr').length == 2)) {
            that['add'+v]();
        }
    },
    init:function(){
        ...
        that.ctype(that.type, null, true);
        if (!that.detail || $.isEmptyObject(that.detail)) {
            that.detail = [[]];
        }
        $.each(that.detail,function(i,v){
            that['add'+that.type](v, i == 0);
        });
    }
};
setTimeout(<?=JVAR?>.init, 500);
</script>