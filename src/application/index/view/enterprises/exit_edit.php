...
$.parser.onComplete = function(){
    ...
    ENTERPRISE_EXIT_EDIT.init();
    if (ENTERPRISE_EXIT_EDIT.readonly) {
        $('#F<?=UNIQID?>').find('.easyui-combobox').combobox('disable');
        $('#F<?=UNIQID?>').find('.easyui-textbox').textbox('disable');
        $('#F<?=UNIQID?>').find('.easyui-datebox').datebox('disable');
        $('#F<?=UNIQID?>').find('.easyui-numberbox').numberbox('disable');
    }
}
var ENTERPRISE_EXIT_EDIT = {
    ...
    uploaded:function(files){
        ...
        var that = ENTERPRISE_EXIT_EDIT;
        $.each(files, function(i,v){
            that.pendingFiles.push(v.attachment_id);
        });
        $('#exit_pending_files').val(that.pendingFiles.join(','));
    },
    init:function(){
        if ('<?=$row['id']?>' == '') {
            return;
    ...
    show:function(v){
        if (v) {
            var $t = $('#T<?=UNIQID?>_'+v);
            var $s = $t.siblings('.e-types');
            $t.removeClass('hidden').find('.easyui-textbox').textbox('enable');
            $t.find('.easyui-numberbox').numberbox('enable');
            $s.addClass('hidden').find('.easyui-textbox').textbox('disable');
            $s.find('.easyui-numberbox').numberbox('disable');
        }
    }
}
GLOBAL.HelperDialog = {
