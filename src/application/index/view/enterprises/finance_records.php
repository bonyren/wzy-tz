...
var <?=JVAR?> = {
        ...
        data.rows.forEach(function(v,i){
            ...
            $(that.datagrid).datagrid('updateRow',{
                ...
                row: {
                    ...
                    valuation:parseInt(v.valuation).toLocaleString(),
                    stock_ratio:v.stock_ratio + '%',
                    shareholders:v.esid == '0' ? '' : '<a href="javascript:void(0)" onclick="<?=JVAR?>.viewes('+v.esid+')">查看</a>',
                    btns:btns.join(' ')
                }
            });
    ...
    viewes:function(esid){
        var url = '<?=url('enterprises/shareholdersEdit')?>?id='+esid+'&readonly=1';
        QT.helper.view( { url:url,width:<?=$loginMobile?"'90%'":800?>,height:'80%',dialog:'view-shareholders' } );
    },
    detailFormatter:function(index,row){
        return '<div class="ddv pd-5"></div>';
    },
    onExpandRow:function(index,row){
        ...
        ddv.panel({
            ...
            border:false,
            cache:false,
            href:'<?=url('Enterprises/getFinancingDetail')?>?efid='+row.id,
            onLoad:function(){
    ...
    edit:function(id){
        var that = this,
            href = '<?=url('enterprises/editFinanceRecord',['enterprise_id'=>$enterprise_id])?>&id='+id;
        QT.helper.dialog('股权变更',href,true,function($dialog){
            ...
            $.post(href, $form.serialize(), function(res){
                ...
                if(!res.code){
                    ...
                    $dialog.dialog('close');
                    that.reload();
                    $('#shareholders_datagrid').datagrid('reload');
                }
            }, 'json');
    ...
    remove:function(id){
        var that = this;
        var url = '<?=url('enterprises/delFinanceRecord')?>';
