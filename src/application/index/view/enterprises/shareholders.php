...
<script>
var <?=JVAR?> = {
    datagrid:'#shareholders_datagrid',
    toolbar:'#<?=TOOLBAR_ID?>',
    ...
    onExpandRow:function(index,row){
        ...
        ddv.panel({
            ...
            border:false,
            cache:false,
            href:'<?=url('Enterprises/getShareholderDetail')?>?esid='+row.id,
            onLoad:function(){
    ...
    edit:function(id){
        var that = this,
            href = '<?=url('Enterprises/shareholdersEdit',['enterprise_id'=>$enterprise_id])?>&id='+id;
        QT.helper.dialog('股东表',href,true,function($dialog){
    ...
    remove:function(id){
        ...
        $.messager.confirm('提示', '确认删除吗?', function(result){
            ...
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('Enterprises/shareholdersRemove')?>', {id:id}, function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
