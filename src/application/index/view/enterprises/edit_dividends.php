<form id="F<?=UNIQID?>">
    <table class="table-form" cellpadding="5">
        ...
        <tr>
            ...
            <td align="center">
                <input name="allocate[<?=$v['fund_id']?>][description]"
                       class="easyui-textbox auto-height" multiline="true"
                       data-options="value:'<?=convertLineBreakToEscapeChars($ed['description'])?>'" style="width:210px;">
            </td>
        </tr>
        <?php endforeach; ?>
    </table>
...
<?php foreach ($funds as $i=>$v): ?>
    $('#fund_dividend_amount_<?=$v['fund_id']?>').fundIncome({
        fundId:'<?=$v['fund_id']?>',
        ffiId:'<?=$ed['ffi_id']?>',
        type:'<?=\app\index\logic\Defs::FUND_INCOME_DIVIDEND_TYPE?>',
        title:'<?=$enterprise['name']?>-项目分红',
        uniqid:md5('#fund_dividend_amount_<?=$v['fund_id']?>')
    });
<?php endforeach; ?>
...
GLOBAL.HelperDialog = {
    saveAmount:function(){
        <?php foreach ($funds as $i=>$v): ?>
            var theId = '#fund_dividend_amount_<?=$v['fund_id']?>';
            var ffiId = $(theId).fundIncome('save');
