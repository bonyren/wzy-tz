...
<script type="text/javascript">
    $('td.statustd').remove();
    $('.org-content').remove();
    $('.phoneview-wrap').remove();
    $('.search-icons').remove();
    $('.app-copy').remove();
    $('.bottomRight').remove();
    $('.nstatus').replaceWith('<a href="javascript:void(0)" class="add-to-project" onclick="add(this)">+项目</a>');
    $('#searchForm').submit(function(){
        var kw = $.trim($(this).find('input[name="kw"]').val());
        if (kw === '') {
            $(this).find('input[name="kw"]').val('').focus();
            return false;
        }
    });
    function zhugeTrack() {
        var href = $(window.event.target).attr('href');
        if (href) {
            window.open('https://www.qichacha.com' + href);
        }
    ...
    function add(that) {
        ...
        var data = {name:'',boss:'',phone:''};
        var matches = html.match(/\{'内容类型':'.*?','内容名称':'.*?'\}/isg);
        if (matches && matches.length) {
            for (var i in matches) {
                try {
                    var obj = JSON.parse(matches[i].replace(/'/g,'"'));
                    if (obj['内容类型'] == '企业') {
                        data.name = obj['内容名称'].replace(/\s+/sg,'');
                    } else if(obj['内容类型'] == '法定代表人') {
                        data.boss = obj['内容名称'].replace(/\s+/sg,'');
                    }
                } catch (e) {
        ...
        //联系方式
        var matches = html.match(/电话：(.*?)<\/span>/is);
        if (matches && matches[1]) {
            data.phone = matches[1].replace(/\s+/sg,'');
        }
        //地址
        var matches = html.match(/地址：(.*?)<\/p>/is);
        if (matches && matches[1]) {
            data.address = matches[1].replace(/[\s+(<em>)(<\/em>)]/sg,'');
        }
        // console.log(data);
        window.parent.EnterpriseModule.add(data);
    }
</script>
</html>