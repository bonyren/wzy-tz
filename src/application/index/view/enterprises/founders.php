...
var <?=JVAR?> = {
    ...
    convert:function(data){
        ...
        $.each(data.rows, function(i,v){
            records.push(v.contact_id);
            $.get('<?=url('index/Upload/viewAttaches')?>',{
                attachmentType:8,
                externalId:v.contact_id,
        ...
        $.post('<?=$tag_url?>',{record_id:records},function(tags){
            $.each(data.rows, function(i,v){
                var btns = [];
                btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="<?=JVAR?>.edit(' + v.contact_id + ')" title="编辑"><i class="fa fa-pencil-square-o fa-lg"></i></a>');
                ...
                $(that.datagrid).datagrid('updateRow',{
                    index: i,
                    row: {
                        tags:typeof tags[v.contact_id] == "undefined" ? '' : tags[v.contact_id],
    ...
    edit:function(id){
        var that = this,
            href = '<?=url('Contacts/edit')?>?enterprise_id=<?=$enterprise_id?>&id='+id;
        QT.helper.dialog('团队成员',href,true,function(){
            that.reload();
        },900,"90%");
    },
    remove:function(id){
        var that = this;
        var url = '<?=url('enterprises/founderRemove')?>';
        $.messager.confirm('提示', '确认从创始团队中删除该成员吗?', function(result){
