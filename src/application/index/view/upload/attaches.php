            ...
            $.app.method.upload(url, function (obj) {
                ...
                $.each(obj.data,function(i,v){
                    html += '<div id="attach_file_<?=$uniqid?>_' +v.attachment_id + '" class="attach-box">' +
                        '<div class="text-center mt-1"><a title="' + v.name + '" href="javascript:void(0)" onclick="QT.filePreview('+v.attachment_id+')">'+
                        '<img class="avatar size-XXL" src="' + v.thumbnail_url + '" /></a></div>' +
                        '<div class="text-center mt-1 attach-name">' + v.name + '</div>' +
