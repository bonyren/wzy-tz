    ...
    <?php foreach($enterprises as $enterprise){ ?>
        <tr>
            <td class="field-input">
                ...
                <a class="btn btn-outline-info size-MINI radius my-1" href="#" onclick="ddEdit(this, <?=htmlspecialchars(json_encode($enterprise, JSON_UNESCAPED_UNICODE))?>);return false;"><span class="fa fa-pencil"></span></a>
                <?php } ?>
            </td>
...
<script type="text/javascript">
    function ddEdit(that, param){
        console.log(param);
        var redisKey = param.redis_key;
        var href = '<?=url('Dd/enterpriseSave')?>';
        href = GLOBAL.func.addUrlParam(href, 'redisKey', redisKey);

        var postUrl = '<?=url('Redis/setData')?>';
        postUrl = GLOBAL.func.addUrlParam(postUrl, 'key', redisKey);
        var dialogId = '#globel-dialog-div';
        $(dialogId).dialog({
            title: '设置尽调数据' + '-' + param.name,
            ...
            buttons:[{
                ...
                handler: function(){
                    ...
                    $.post(postUrl, jsonStr, function(res){
                        ...
                        if(!res.code){
                            ...
                            //refresh?
                            $(that).closest('.ddv').panel('refresh');
                        }
                    }, 'json');
                }
            },{
