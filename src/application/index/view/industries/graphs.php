...
var <?=JVAR?> = {
    ...
    view:function(idx,row){
        var url = '<?=url('industries/viewGraph')?>?id='+row.id;
        QT.helper.view({
            url:url,
        });
    },
    edit:function(id){
        var that = this;
        var href = '<?=url('industries/editGraph')?>?id='+(id?id:'');
    ...
    remove:function(id){
        ...
        $.messager.confirm('提示', '确认删除吗?', function(result){
            ...
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('industries/deleteGraph')?>', {id:id}, function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
