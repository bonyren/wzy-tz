...
var sysErrExpModule={dialog:'#globel-dialog-div',datagrid:'#sysErrExpDatagrid',reload:function(){$(this.datagrid).datagrid('reload');},reset:function(){var that=this;$(that.datagrid).datagrid('load',{});},clear:function(){$.messager.confirm('提示','确定清空吗?',function(result){if(!result)return false;$.messager.progress({text:'处理中，请稍候...'});$.post('<?=url('index/System/clearSysErrExp')?>',function(res){$.messager.progress('close');if(res.code){$.app.method.tip('提示',res.msg,'info');sysErrExpModule.reset();}else{$.app.method.alertError(null,res.msg);}},'json');});}};</script>