...
<div class="easyui-layout" data-options="fit:true">
    <div data-options="region:'north',collapsible:false,border:false" style="height:60%;">
        <table id="progressLogsDatagrid_<?=$uniqid?>" class="easyui-datagrid" data-options="striped:true,
            ...
            <tr>
                <?php if(!$readOnly){ ?>
                <th data-options="field:'operate',width:100,align:'center',formatter:progressLogsModule_<?=$uniqid?>.operate">操作</th>
                <?php } ?>
                <th data-options="field:'occur_date',width:200">日期</th>
                ...
                <?php endif; ?>
                <th data-options="field:'files',width:200">附件</th>
...
<?php if(!$readOnly){ ?>
    <div class="form-compact-container">
        <form id="progressLogsAddForm_<?=$uniqid?>" method="post" class="form-compact-body">
...
var progressLogsModule_<?=$uniqid?>={progressLogsAttacheIds:[],dialog:'#globel-dialog-div',datagrid:'#progressLogsDatagrid_<?=$uniqid?>',onLoaded:function(data){var that=progressLogsModule_<?=$uniqid?>;data.rows.forEach(function(v,i){$.get('<?=url('index/Upload/viewAttaches')?>',{attachmentType:'<?=\app\index\logic\Upload::ATTACH_PROGRESS_LOGS?>',externalId:v.progress_log_id,uiStyle:'<?=\app\index\controller\Upload::ATTACHES_UI_LINK_STYLE?>'},function(res){$(that.datagrid).datagrid('updateRow',{index:i,row:{title:'<button type="button" class="btn btn-link" onclick="progressLogsModule_<?=$uniqid?>.view('+v.progress_log_id+')">'+v.title+'</button>',files:res}});});})},operate:function(val,row){var btns=[];btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="progressLogsModule_<?=$uniqid?>.edit('+row.progress_log_id+')" title="编辑"><i class="fa fa-pencil-square-o fa-lg"></i></a>');btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="progressLogsModule_<?=$uniqid?>.del('+row.progress_log_id+')" title="删除"><i class="fa fa-trash-o fa-lg"></i></a>');return btns.join(' ');},formatShowTimeline:function(val,row){if(val==1){return'<span class="fa fa-check"></span>';}else{return'';}},reload:function(){$(this.datagrid).datagrid('reload');},load:function(){$(this.datagrid).datagrid('load');},search:function(){var that=this;var queryParams=$(that.datagrid).datagrid('options').queryParams;$.each($("#progressLogsToolbarForm_<?=$uniqid?>").serializeArray(),function(){delete queryParams[this['name']];});$.each($("#progressLogsToolbarForm_<?=$uniqid?>").serializeArray(),function(){queryParams[this['name']]=this['value'];});that.load();},reset:function(){var that=this;$("#progressLogsToolbarForm_<?=$uniqid?>").form('reset');var queryParams=$(that.datagrid).datagrid('options').queryParams;for(var key in queryParams){delete queryParams[key];}
that.load();},add:function(){var that=this;var href='<?=$urlHrefs['add']?>';$('#progressLogsAddForm_<?=$uniqid?>').form('submit',{url:href,iframe:false,onSubmit:function(){var isValid=$(this).form('validate');if(!isValid)return false;$.messager.progress({text:'处理中，请稍候...'});return true;},success:function(data){var res=eval('('+data+')');$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');$("#progressLogsAddForm_<?=$uniqid?>").form('reset');that.load();progressLogsModule_<?=$uniqid?>.progressLogsAttacheIds.splice(0,progressLogsModule_<?=$uniqid?>.progressLogsAttacheIds.length);$("#progressLogsAttachsPanel_<?=$uniqid?>").panel('refresh');}}});},edit:function(id){var that=this;var href='<?=url('ProgressLogs/edit')?>?src=<?=$src?>&id='+id;QT.helper.dialog('修改',href,true,function($dialog){var $form=$dialog.find('form:eq(0)');if(!$form.form('validate')){return false;}
$.messager.progress({text:'处理中，请稍候...'});$.post(href,$form.serialize(),function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg);$dialog.dialog('close');that.reload();}},'json');},<?=$loginMobile?"'90%'":800?>,"95%",'progress_log_edit_dialog');},del:function(progressLogId){var that=this;var href='<?=$urlHrefs['delete']?>';href+=href.indexOf('?')!=-1?'&progressLogId='+progressLogId:'?progressLogId='+progressLogId;$.messager.confirm('提示','确认删除吗?',function(result){if(!result)return false;$.messager.progress({text:'处理中，请稍候...'});$.post(href,{},function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');that.reload();}},'json');});},view:function(id){QT.helper.view({url:'<?=url('ProgressLogs/view')?>?id='+id,width:<?=$loginMobile?"'90%'":800?>,height:'95%',dialog:'progress_log_view_dialog'});},onAttachmentsUploaded:function(files){$.each(files,function(i,v){progressLogsModule_<?=$uniqid?>.progressLogsAttacheIds.push(v.attachment_id);});$('#progressLogsAttacheIds_<?=$uniqid?>').val(progressLogsModule_<?=$uniqid?>.progressLogsAttacheIds.join(','));}};$.parser.onComplete=function(context){$('#entry_<?=$uniqid?>').textbox('autoHeight');$.parser.onComplete=$.noop;}</script>