...
<script>
var <?=JVAR?>={progressLogsAttacheIds:[],subtypes:<?=json_encode($subtypes,JSON_UNESCAPED_UNICODE)?>,onAttachmentsUploaded:function(files){$.each(files,function(i,v){<?=JVAR?>.progressLogsAttacheIds.push(v.attachment_id);});$('#progressLogsAttacheIds_<?=$uniqid?>').val(<?=JVAR?>.progressLogsAttacheIds.join(','));},onSubtypeSelected:function(value){var that=<?=JVAR?>;$('#progressLogsAddForm_<?=$uniqid?>').find('.progress_logs_extra_inputs').each(function(){if(value!=''&&that.subtypes[value].flag==$(this).attr('flag')){$(this).removeClass('hidden');}else{$(this).addClass('hidden');}});if(value==''||that.subtypes[value].flag!='meeting_res'){$('#MR<?=JVAR?>').combobox('reset');}}};$.parser.onComplete=function(context){var txtbox=$(".auto-height");if(txtbox.length){$.each(txtbox,function(i,v){$(v).textbox('autoHeight');})}
