...
var choosePartnerModule={datagrid:'#choosePartnerDatagrid',choosePartner:{pId:0,name:''},formatStatus:function(val,row){return'<span class="badge badge-success">有效</span>';},formatType:function(val,row){return<?=json_encode(\app\index\logic\Defs::$partnerTypeHtmlDefs,JSON_UNESCAPED_UNICODE)?>[val];},reload:function(){$(this.datagrid).datagrid('reload');},load:function(){$(this.datagrid).datagrid('load');},search:function(){var that=this;var queryParams=$(that.datagrid).datagrid('options').queryParams;$.each($("#choosePartnerToolbarForm").serializeArray(),function(){delete queryParams[this['name']];});$.each($("#choosePartnerToolbarForm").serializeArray(),function(){queryParams[this['name']]=this['value'];});that.load();},reset:function(){var that=this;var queryParams=$(that.datagrid).datagrid('options').queryParams;for(var key in queryParams){delete queryParams[key];}
$("#choosePartnerToolbarForm").form('reset');that.load();},onSelect:function(index,row){var that=this;that.choosePartner.pId=row.p_id;that.choosePartner.name=row.name;$("#choosePartnerName").text(row.name);}};</script>