<div class="daohe-panel daohe-panel-default" style="border-color:#fff;">
    ...
    <div class="daohe-panel-body">
        <table class="daohe-table daohe-table-border daohe-table-bg mt-1 mb-1">
            <thead>
                <tr>
                    <th>文件名</th>
                    <!--
            ...
            <?php foreach($bindValues['attaches'] as $attach){ ?>
                <tr id="attach_file_<?=$uniqid?>_<?=$attach['attachment_id']?>">
                    ...
                    <td>
                        <a class="text-secondary size-MINI" href="<?=$attach['download_url']?>" target="_blank"><span class="fa fa-download"></span></a>
                    </td>
                    <td>
                        <a class="text-danger size-MINI" href="javascript:void(0)" onclick="attachModule_<?=$uniqid?>.deleteAttach(<?=$attach['attachment_id']?>)"><span class="fa fa-remove"></span></a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
...
var attachModule_<?=$uniqid?>={dialog:'#globel-dialog-div',dialog2:'#globel-dialog2-div',callback:<?=$bindValues['callback']?$bindValues['callback']:'null'?>,queries:<?=json_encode($_GET,JSON_UNESCAPED_UNICODE)?>,uploadAttach:function(){var that=this;var url='<?=$urlHrefs['uploadAttach']?>';$.app.method.upload(url,function(obj){if(!obj.code){$.app.method.tip('提示',obj.msg,'error');return;}else{if(obj.html){$.messager.alert('提示',obj.html,'warning');}else{$.app.method.tip('提示','成功','info');}}
var html='';$.each(obj.data,function(i,v){html+='<tr id="attach_file_<?=$uniqid?>_'+v.attachment_id+'">'+'<td><a title="'+v.name+'" href="javascript:void(0)" onclick="QT.filePreview('+v.attachment_id+')">'+
v.name+'</a></td>'+'<td >'+v.entered+'</td>'+'<td><a class="text-secondary size-MINI" href="'+v.download_url+'" target="_blank"><span class="fa fa-download"></span></a></td>'+'<td><a class="text-danger size-MINI" href="javascript:void(0)" onclick="attachModule_<?=$uniqid?>.deleteAttach('+v.attachment_id+')"><span class="fa fa-remove"></span></a></td>'+'</tr>';});$("#attaches_<?=$uniqid?>").append(html);if(that.callback){that.callback(obj.data,that.queries);}});},deleteAttach:function(attachmentId){var that=this;var href='<?=$urlHrefs['deleteAttach']?>';href+=href.indexOf('?')!=-1?'&attachmentId='+attachmentId:'?attachmentId='+attachmentId;$.messager.confirm('提示','确认删除吗?',function(result){if(!result)return false;$.messager.progress({text:'处理中，请稍候...'});$.post(href,{},function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');$('#attach_file_<?=$uniqid?>_'+attachmentId).remove();}},'json');});}};</script>