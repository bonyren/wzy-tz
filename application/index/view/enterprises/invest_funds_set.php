...
GLOBAL.HelperDialog={init:function(){<?php  foreach($funds as $i=>$v):?>$('#fund_enterprise_amount_<?=$v['fund_id']?>').fundEnterprise({fundId:'<?=$v['fund_id']?>',ffeId:0,enterpriseId:'<?=$enterprise_id?>',title:'<?=$enterprise['name']?>-投资交割',uniqid:md5('#fund_enterprise_amount_<?=$v['fund_id']?>')});<?php  endforeach;?>},saveAmount:function(){<?php  foreach($funds as $i=>$v):?>var theId='#fund_enterprise_amount_<?=$v['fund_id']?>';var ffiId=$(theId).fundEnterprise('save');$(theId+'_input').val(ffiId);<?php  endforeach;?>},submit:function(url,$dialog,success){var $form=$('#invest_funds_set_form');if(!$form.form('validate')){return false;}
GLOBAL.HelperDialog.saveAmount();$.messager.progress({text:'处理中，请稍候...'});$.post(url,$form.serialize(),function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');$dialog.dialog('close');if(success){success();}}},'json');},uploaded:function(files,queries){var input=$('#transfer_directive_fund_'+queries.fund_id);var fids=[];$.each(files,function(i,v){fids.push(v.attachment_id);});fids=fids.join(',');var old=input.val();var ids=old?(old+','+fids):fids;input.val(ids);}};GLOBAL.HelperDialog.init();</script>