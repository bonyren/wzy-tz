...
var creditSearchModule={dialog:'#globel-dialog-div',datagrid:'#creditSearchDatagrid',searchForm:'#creditSearchForm',formatOpt:function(val,row){var btns=[];btns.push('<a href="javascript:;" class="btn btn-outline-info size-MINI radius my-1" onclick="creditSearchModule.add(\''+encodeURIComponent(JSON.stringify(row))+'\')" title="添加"><i class="fa fa-plus fa-lg">添加</i></a>');return btns.join(' ');},reload:function(){$(creditSearchModule.datagrid).datagrid('reload');},search:function(){var param={};$.each($(this.searchForm).serializeArray(),function(){param[this.name]=this.value;});$(this.datagrid).datagrid('load',param);},reset:function(){$(this.searchForm).form('reset');$(this.datagrid).datagrid('load',{});},add:function(row){var row=JSON.parse(decodeURIComponent(row));EnterpriseModule.add({name:row.companyName,boss:row.legalPerson,phone:''});}};</script>