<div class="form-container">
    <form id="companies_edit_form" method="post" class="form-body">
        <table class="table-form" cellpadding="5">
            <tr>
                <td class="field-label" style="width: 150px;">公司名称</td>
                <td>
            ...
            <tr>
                ...
                <td>
                    <textarea class="easyui-textbox auto-height" name="data[introduction]" data-options="
                        width:'100%',
                        multiline:true,
                        validType:['length[1,4096]']"><?=$data['introduction']??''?></textarea>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <input class="easyui-combobox" name="data[assigner]" style="width:100%;;"
                        data-options="
                            required:false,
                            editable:false,
                            value:'<?=!empty($data['assigner'])?$data['assigner']:''?>',
...
<script>
var <?=JVAR?>={pendingFiles:[],uploaded:function(files){<?php  if(empty($data['id'])):?>var that=<?=JVAR?>;$.each(files,function(i,v){that.pendingFiles.push(v.attachment_id);});$('#company_pending_files').val(that.pendingFiles.join(','));<?php  endif;?>},save:function(){var $form=$('#companies_edit_form');if(!$form.form('validate')){return false;}
$.messager.progress({text:'处理中，请稍候...'});var $data=$form.serialize();$.post('<?=$_request_url?>',$data,function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg);$('#globel-dialog-div').dialog('close');$('#<?=$grid?>').datagrid('reload');}},'json');},cancel:function(){$('#globel-dialog-div').dialog('close');}};$.parser.onComplete=function(){var txtbox=$(".auto-height");if(txtbox.length){$.each(txtbox,function(i,v){$(v).textbox('autoHeight');})}
