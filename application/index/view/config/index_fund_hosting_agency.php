...
<table id="fundHostingAgencyDatagrid" data-options="striped:true,
    nowrap:false,
    rownumbers:true,
    autoRowHeight:true,
    singleSelect:true,
...
var fundHostingAgencyModule={dialog:'#globel-dialog-div',datagrid:'#fundHostingAgencyDatagrid',toolbar:[{text:'添加',iconCls:iconClsDefs.add,handler:function(){fundHostingAgencyModule.add();}},{text:'刷新',iconCls:'fa fa-refresh',handler:function(){fundHostingAgencyModule.refresh();}}],operate:function(val,row,index){var btn=[];btn.push('<a href="javascript:;" onclick="fundHostingAgencyModule.edit('+row.id+')"><i class="fa fa-pencil-square-o fa-lg"></i></a>');btn.push('<a href="javascript:;" onclick="fundHostingAgencyModule.delete('+row.id+')"><i class="fa fa-trash-o fa-lg"></i></a>');return btn.join(' | ');},refresh:function(){$(this.datagrid).datagrid('reload');},reload:function(){$(this.datagrid).datagrid('reload');},load:function(){$(this.datagrid).datagrid('load');},reset:function(){var that=this;var queryParams=$(that.datagrid).datagrid('options').queryParams;for(var key in queryParams){delete queryParams[key];}
that.load();},add:function(){var that=this;var href='<?=$urlHrefs['add']?>';$(that.dialog).dialog({title:'添加基金托管机构',iconCls:iconClsDefs.add,width:<?=$loginMobile?"'90%'":600?>,height:'95%',cache:false,href:href,modal:true,collapsible:false,minimizable:false,resizable:false,maximizable:false,buttons:[{text:'确定',iconCls:iconClsDefs.ok,handler:function(){$(that.dialog).find('form').eq(0).form('submit',{url:href,iframe:false,onSubmit:function(){var isValid=$(this).form('validate');if(!isValid)return false;$.messager.progress({text:'处理中，请稍候...'});return true;},success:function(data){console.log(data);var res=eval('('+data+')');$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');$(that.dialog).dialog('close');that.reload();}}});}},{text:'取消',iconCls:iconClsDefs.cancel,handler:function(){$(that.dialog).dialog('close');}}]});$(that.dialog).dialog('center');},edit:function(id){var that=this;var href='<?=$urlHrefs['edit']?>';href+=href.indexOf('?')!=-1?'&id='+id:'?id='+id;$(that.dialog).dialog({title:'修改基金托管机构',iconCls:iconClsDefs.edit,width:<?=$loginMobile?"'90%'":600?>,height:'95%',cache:false,href:href,modal:true,collapsible:false,minimizable:false,resizable:false,maximizable:false,buttons:[{text:'确定',iconCls:iconClsDefs.ok,handler:function(){$(that.dialog).find('form').eq(0).form('submit',{url:href,iframe:false,onSubmit:function(){var isValid=$(this).form('validate');if(!isValid)return false;$.messager.progress({text:'处理中，请稍候...'});return true;},success:function(data){var res=eval('('+data+')');$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');$(that.dialog).dialog('close');that.reload();}}});}},{text:'取消',iconCls:iconClsDefs.cancel,handler:function(){$(that.dialog).dialog('close');}}]});$(that.dialog).dialog('center');},delete:function(id){var that=this;var href='<?=$urlHrefs['delete']?>';href+=href.indexOf('?')!=-1?'&id='+id:'?id='+id;$.messager.confirm('提示','确认删除吗?',function(result){if(!result)return false;$.messager.progress({text:'处理中，请稍候...'});$.post(href,{},function(res){$.messager.progress('close');if(!res.code){$.app.method.alertError(null,res.msg);}else{$.app.method.tip('提示',res.msg,'info');that.reload();}},'json');});}};var options=<?=json_encode($datagrid['options'])?>;for(var key in options){if(key=='toolbar'){options[key]=fundHostingAgencyModule.toolbar;}}
$('#fundHostingAgencyDatagrid').datagrid(options);</script>