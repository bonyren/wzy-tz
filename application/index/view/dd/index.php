...
<script>
var ddIndexModule={dialog:'#globel-dialog-div',datagrid:'#ddIndexDatagrid',detailFormatter:function(index,row){return'<div class="ddv" style="overflow:auto;"></div>';},expandDetailRow:function(index,row){var that=this;var ddv=$(that.datagrid).datagrid('getRowDetail',index).find('div.ddv');var href="";var width=600;<?php  if($view==Dd::GLOBAL_PARTNERS_VIEW){?>href='<?=url('index/Dd/partners')?>';<?php }else if($view==Dd::GLOBAL_ENTERPRISES_VIEW){?>href='<?=url('index/Dd/enterprises')?>';width='100%';<?php }?>href+=href.indexOf('?')!=-1?'&fundId='+row.fund_id:'?fundId='+row.fund_id;ddv.panel({width:width,height:250,border:false,cache:false,href:href,onResize:function(){},onLoad:function(){}});$(that.datagrid).datagrid('fixDetailRowHeight',index);},};</script>