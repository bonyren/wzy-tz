...
<div id="p2" class="easyui-navpanel">
    <header>
        <div class="m-toolbar">
            ...
            <div class="m-left">
                <a href="javascript:;" class="easyui-linkbutton m-back" data-options="plain:true,back:true">后退</a>
            </div>
        </div>
    </header>
...
<script type="text/javascript">
var mApp={dialog:'#globel-dialog-div',init:function(){},go:function(title,url){$('#m-title-p2').text(title);$('#m-content-p2').panel('refresh',url);$.mobile.go('#p2');},open:function(title,url){if(!url){return false;}
$('#m-content').panel('setTitle',title).panel('refresh',url);},view:function(options){$(mApp.dialog).dialog({title:options.title,width:"95%",height:'90%',href:options.url,modal:true,border:false,buttons:[{text:'关闭',iconCls:'fa fa-close',handler:function(){$(mApp.dialog).dialog('close');}}]});$(mApp.dialog).dialog('center');},updateCss:function(){},logout:function(){$.messager.confirm('提示信息','确定退出登录吗？',function(y){if(!y){return;}
$.messager.progress({text:'处理中，请稍候...'});$.post('<?=url('index/logout')?>',function(data){$.messager.progress('close');if(data.code){window.location.href=data.data;}});});}};$(function(){mApp.init();});$(document).ajaxError(function(event,XMLHttpRequest,ajaxOptions){switch(XMLHttpRequest.status){case 401:window.location.assign('<?=url('lp/Index/login')?>');break;case 400:$.app.method.alertError(null,"错误的请求");if(ajaxOptions.type=='POST'){$.messager.progress('close');}
